...
public class MBSItem extends Item {
            ...
            if (!user.getItemCooldownManager().isCoolingDown(stack)) {
                ...
                if (count == 2) {
                    ...
                    for (int i = 1; i <= 10; i++) {
                        ...
                        int color = 0xFFEB00;
                        //(ServerPlayerEntity) user, new DustParticleEffect(color, 1.0f
                        ((ServerWorld) world).spawnParticles(new DustParticleEffect(color, 1f), true, true, getBlockInFrontX(user, i), getBlockInFrontY(user, 0), getBlockInFrontZ(user, i), 200, 0.5, 0.5, 0.5, 0.2);
                    }
                ...
                if (count == 3) {
                    ...
                    int color = 0x00FFE0;
                    ((ServerWorld) world).spawnParticles(new DustParticleEffect(color, 1.0f), true, true, user.getX(), user.getY(), user.getZ(), 1000, 5, 0.1, 5, 1);
                }
            }
        }
        return ActionResult.SUCCESS;
    ...
    public void inventoryTick(ItemStack stack, ServerWorld world, Entity entity, @Nullable EquipmentSlot slot) {
        if (entity instanceof PlayerEntity player && !world.isClient && world instanceof ServerWorld serverWorld) {
            ...
            double radius = 0;
            for (LivingEntity livingEntity : (serverWorld).getEntitiesByClass(LivingEntity.class, player.getBoundingBox().expand(30), e -> true)) {
            ...
            if (lazerDuration > 1) {
                for (int i = 3; i <= 18; i++) {
                    if (world instanceof ServerWorld serverWorld1){
                        ...
                        for (InteractionEntity iEntity : world.getEntitiesByClass(InteractionEntity.class, player.getBoundingBox().expand(teleportRadius), e -> true)) {
                            if (iEntity.squaredDistanceTo(player) <= teleportRadius * teleportRadius) {
                                if(hasTag(iEntity, "lazer" + i)) {
                                    iEntity.teleport(serverWorld1, getBlockInFrontX(player, i), getBlockInFrontY(player, i), getBlockInFrontZ(player, i), EnumSet.noneOf(PositionFlag.class), player.getYaw(), 1f, false);
                                }
                            }
                        }
                    }
